...
namespace SpaceWatch.Core.Services.DefaultArea
{
	public class ContentForUserService : IContentForUserService
	{
		...
		public async Task AddComment(CommentAddViewModel model)
		{
			try
			{
				var contentExists = await _repo.AllReadonly<Content>().AnyAsync(c => c.Id == model.ContentId && c.IsActive);
				var categoryItemExists = await _repo.AllReadonly<CategoryItem>().AnyAsync(c => c.Id == model.CategoryItemId && c.IsActive);
				var userExists = await _repo.AllReadonly<ApplicationUser>().AnyAsync(c => c.Id == model.UserId);
